...
class ConfCenter(object):
    ...
    def get(self, key, defval):
        ...
        if key[:3] in ["i:/", "s:/"]:
            ...
            if val is not None:
                print cp.r("CC.WARN(0):"), "Default '%s'='%s'" % (key, val)
                self.setconfref(key)
                return val

            self.dicset(self._dict_conf_missed, key, defval)
            print cp.r("CC.WARN(1):"), "Default '%s'='%s'" % (key, defval)
            return defval
        ...
        print cp.r("CC.WARN(2):"), "Default '%s'='%s'" % (key, defval)
        return defval
...
class XConfCenter(object):
    ...
    def __init__(self, group, ro_cfgs=None, rw_cfg=None, conf=None):
